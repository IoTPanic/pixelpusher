...
func applyRouted(r *mux.Router) {
	...
	r.HandleFunc("/api/user/{UID}", protect(handleUserRequest)).Methods("GET", "PUT", "DELETE")
	...
	r.HandleFunc("/api/project/{PID}/device/{DID}", protect(handleDeviceRequest)).Methods("GET", "PUT", "DELETE")
